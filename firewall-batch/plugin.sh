...
JS_FILE="firewall-batch.js"
JS_DEST="/usr/share/pve-manager/js/$JS_FILE"
SCRIPT_MARKER="<!-- PVE_FIREWALL_BATCH_SCRIPT -->"
PM_PLUGIN_NAME="firewall-batch"
...
marker = '<!-- PVE_FIREWALL_BATCH_SCRIPT -->'
script = marker + '\n<script src="/pve2/js/firewall-batch.js"></script>'

text = text.replace('<script src="/pve2/js/firewall-batch.js"></script>\n', '')
text = text.replace('<script src="/pve2/js/firewall-batch.js"></script>', '')
...
template = Path('/usr/share/pve-manager/index.html.tpl')
text = template.read_text()
text = text.replace('<!-- PVE_FIREWALL_BATCH_SCRIPT -->\n<script src="/pve2/js/firewall-batch.js"></script>\n', '')
text = text.replace('<!-- PVE_FIREWALL_BATCH_SCRIPT -->\n<script src="/pve2/js/firewall-batch.js"></script>', '')
text = text.replace('<script src="/pve2/js/firewall-batch.js"></script>\n', '')
text = text.replace('<script src="/pve2/js/firewall-batch.js"></script>', '')
template.write_text(text)
